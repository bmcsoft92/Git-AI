...
const supabaseUrl = Deno.env.get('SUPABASE_URL') || 'https://jyimtfbfntikeugqumxb.supabase.co'
...
serve(async (req) => {
  ...
  try {
    ...
    if (userEmail && diagnosticData && supabase) {
      try {
        ...
        if (insertError) {
          ...
          // Utiliser fetch direct pour l'email
